# Clean Insight Carousel Generator - Project Instructions
...
This version uses the Clean Insight style.

Use this style when the user wants a carousel that feels:
- clear
- structured
- readable
- educational but polished
...
- frameworks
- common mistakes
- problem-solution or transformation carousels

Do not drift into a news-heavy editorial cover or a hard-sell promo carousel.
...
If the user gives no cover image, generate a clean, structured hero treatment that still feels strong.
...
Clean Insight accent treatment:
- Keep contrast more even across the carousel than Overlay Hook
- Use accents to clarify sections, cards, or steps rather than to create drama
- Strong gradient or accent usage should support readability, not overpower it
...
For Clean Insight, prioritize pairings that feel clean, readable, structured, and calm.
...
- Bottom-aligned content that clears the progress bar: `0 38px 54px`
- Prefer cleaner vertical grouping and more consistent section spacing
- Supporting slides can use structured lists or cards when that improves explanation clarity

### Clean Insight Cover Treatment
- Prefer a clear text-led cover or a lightly supported image
- Use calmer backgrounds, soft panels, subtle gradients, or diagram-like structure
- If an image is used, treat it as supportive rather than the dominant full-bleed hero
- Place the main hook in the upper-middle or middle area for fast reading
- Keep the hierarchy strong, but less dramatic than Overlay Hook
- Use a short tag or framing line when it helps orient the topic

If no cover image is available:
- Create a structured CSS gradient or panel-based fallback
- Use subtle shapes, dividers, or tonal blocks rather than heavy atmospheric effects
- Keep the result elegant, readable, and intentional
- Never leave the cover blank or placeholder-like

### Clean Insight Supporting Slide Feel
- Slides 2+ should feel orderly, highly readable, and explanation-friendly
- Each slide should carry one clear idea, step, or principle
- Use cards, lists, dividers, or numbered structure when helpful
- Keep copy concise but allow enough room to explain properly
- Prioritize reading comfort over visual drama
...
### Tag / Category Label
```html
<span class="sans" style="display:inline-flex;align-items:center;padding:6px 10px;border:1px solid {LIGHT_BORDER};border-radius:999px;font-size:10px;font-weight:600;letter-spacing:1.5px;color:{BRAND_DARK};background:rgba(255,255,255,0.72);margin-bottom:16px;">{TAG TEXT}</span>
```

### Callout Card
```html
<div style="padding:16px 18px;background:rgba(255,255,255,0.78);border-radius:14px;border:1px solid {LIGHT_BORDER};box-shadow:0 10px 24px rgba(0,0,0,0.05);">
  <p class="sans" style="font-size:11px;font-weight:600;letter-spacing:1.4px;text-transform:uppercase;color:#8A8580;margin-bottom:8px;">{Label}</p>
  <p class="sans" style="font-size:14px;color:{DARK_BG};font-weight:600;line-height:1.5;">{Callout text}</p>
</div>
```

### Step Row / Structured List
```html
<div style="display:flex;align-items:flex-start;gap:12px;padding:14px 0;border-bottom:1px solid {LIGHT_BORDER};">
  <div style="width:28px;height:28px;border-radius:999px;background:{BRAND_LIGHT};color:{BRAND_DARK};display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;">{n}</div>
  <div>
    <span class="sans" style="display:block;font-size:14px;font-weight:600;color:{DARK_BG};margin-bottom:4px;">{Label}</span>
    <span class="sans" style="font-size:12px;color:#8A8580;line-height:1.5;">{Description}</span>
  </div>
</div>
```
...
```html
<div style="display:inline-flex;align-items:center;gap:8px;padding:12px 22px;background:#fff;color:{BRAND_DARK};font-weight:600;font-size:14px;border-radius:14px;border:1px solid {LIGHT_BORDER};box-shadow:0 10px 24px rgba(0,0,0,0.06);">
  {CTA text}
</div>
```
...
1. Hook / clear promise
2. Problem / common mistake
3. Why it happens
4. What changes it
5. Step or principle
6. Example / outcome / next step
7. CTA / takeaway

Rules:
- Start with a clear hook - the first slide must be immediately understandable and worth the swipe
...
### Clean Insight CTA Tone
- Keep the CTA practical and calm
- Good CTA directions: save this, try this, apply this, use this, follow for more
- Avoid dramatic closing copy or hard-sell pressure
...
## Clean Insight Quality Filter

Before finalizing the carousel, check:
1. Does slide 1 feel clear, structured, and polished rather than dramatic?
2. Is the main promise easy to read quickly?
3. If no image was used, does the cover still feel intentional and complete?
4. Do slides 2+ feel organized, calm, and explanation-friendly?
5. Does each slide focus on one clear idea, step, or explanation block?
...
10. Does the CTA feel practical and useful?
...
- cover feels too dramatic, newsy, or cinematic
- weak explanation hierarchy on slides 2+
- too much body copy packed into a single slide
- using decorative styling where clearer structure is needed
- turning the carousel into a plain text wall with no visual guidance
